...
// Check multiple common git locations on cPanel servers
$commonGitPaths = array(
	'/usr/local/cpanel/3rdparty/lib/path-bin',  // Standard cPanel location
	'/usr/local/bin',                            // Common system location
	'/usr/bin',                                  // Standard system location
	'/opt/cpanel/ea-git-core/bin',              // EasyApache 4 git location
	'/usr/local/cpanel/3rdparty/bin',           // Alternative cPanel location
);
$currentPath = getenv('PATH') ?: '';
$pathsToAdd = array();
foreach ($commonGitPaths as $path) {
	if (is_dir($path) && strpos($currentPath, $path) === false) {
		$pathsToAdd[] = $path;
	}
}
if (!empty($pathsToAdd)) {
	$newPath = implode(PATH_SEPARATOR, array_merge($pathsToAdd, array($currentPath)));
	putenv('PATH=' . $newPath);
}
...
// Helper function to find binary in common locations
function findBinary($command) {
	...
	$pathResult = @shell_exec('which '.$command.' 2>/dev/null');
...
// Store binary paths for later use
$binaryPaths = array();

foreach ($requiredBinaries as $command) {
	// Extract just the command name (handle cases like "composer --no-ansi")
	$cmdParts = explode(' ', $command);
	$cmdName = $cmdParts[0];
	
	$path = findBinary($cmdName);
	if ($path == '') {
		header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error', true, 500);
		die(sprintf('<div class="error"><b>%s</b> not available. It needs to be installed on the server for this script to work.<br>Checked common locations: /usr/local/cpanel/3rdparty/lib/path-bin, /usr/bin, /usr/local/bin, /opt/cpanel/ea-git-core/bin</div>', $cmdName));
	} else {
		// Store the path for this command
		$binaryPaths[$cmdName] = $path;
		
		// Use full path for version check
		$versionOutput = @shell_exec($path.' --version 2>/dev/null');
		$version = is_string($versionOutput) ? explode("\n", trim($versionOutput)) : array('unknown version');
		printf('<b>%s</b> : %s'."\n"
			, $path
			, $version[0]
		);
...
// Get git path for use in commands
$gitPath = isset($binaryPaths['git']) ? $binaryPaths['git'] : 'git';
?>
...
if (!is_dir(TMP_DIR)) {
	...
	$commands[] = sprintf(
		'%s --git-dir="%s.git" --work-tree="%s" fetch --tags origin %s'
		, escapeshellarg($gitPath)
		, TMP_DIR
		, TMP_DIR
		, BRANCH
