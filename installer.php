...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($step === 2) {
        ...
        // Download deploy.php
        $deployPhpUrl = 'https://raw.githubusercontent.com/markomarkovic/simple-php-git-deploy/master/deploy.php';
        ...
        if ($deployPhpContent !== false) {
            if (file_put_contents($currentDir . '/deploy.php', $deployPhpContent)) {
                $success[] = 'deploy.php downloaded successfully';
            } else {
        ...
        // Download deploy-config.example.php
        $configExampleUrl = 'https://raw.githubusercontent.com/markomarkovic/simple-php-git-deploy/master/deploy-config.example.php';
        $configExampleContent = @file_get_contents($configExampleUrl);
        if ($configExampleContent === false && function_exists('curl_init')) {
            $ch = curl_init($configExampleUrl);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            $configExampleContent = curl_exec($ch);
            curl_close($ch);
        }
        
        if ($configExampleContent !== false) {
            if (file_put_contents($currentDir . '/deploy-config.example.php', $configExampleContent)) {
                $success[] = 'deploy-config.example.php downloaded successfully';
            } else {
                $errors[] = 'Failed to write deploy-config.example.php - check file permissions';
            }
        } else {
            $errors[] = 'Failed to download deploy-config.example.php';
        }
        ...
        if (empty($errors)) {
            ...
            $configExampleFile = __DIR__ . '/deploy-config.example.php';
            if (!file_exists($configExampleFile)) {
                $errors[] = 'deploy-config.example.php not found. Please download it first.';
            } else {
                $configContent = file_get_contents($configExampleFile);
                
                // Replace the key configuration values
                $configContent = preg_replace(
                    "/define\s*\(\s*['\"]SECRET_ACCESS_TOKEN['\"]\s*,\s*['\"].*?['\"]\s*\)/",
                    "define('SECRET_ACCESS_TOKEN', '" . addslashes($secretToken) . "')",
                    $configContent
                );
                
                $configContent = preg_replace(
                    "/define\s*\(\s*['\"]REMOTE_REPOSITORY['\"]\s*,\s*['\"].*?['\"]\s*\)/",
                    "define('REMOTE_REPOSITORY', '" . addslashes($repoUrl) . "')",
                    $configContent
                );
                
                $configContent = preg_replace(
                    "/define\s*\(\s*['\"]BRANCH['\"]\s*,\s*['\"].*?['\"]\s*\)/",
                    "define('BRANCH', '" . addslashes($branch) . "')",
                    $configContent
                );
                
                $configContent = preg_replace(
                    "/define\s*\(\s*['\"]TARGET_DIR['\"]\s*,\s*['\"].*?['\"]\s*\)/",
